...
void Book1::Load()
{
    ...
    if (!SoundFlag) {
        SoundEffect::PlaySoundEffect(SFX::Book1BGM);
        SoundEffect::Loop_SoundEffect(SFX::Book1BGM);
        SoundEffect::SetSoundEffectVolume(SFX::Book1BGM, background_sound);
        SoundFlag = true;
    }
    ...
    GetGSComponent<Background>()->Add("assets/sprite/Background/background.spt", 1, Engine::Instance()->Get_Window() * 2);

    player_ptr = new Player({ 0, -200 }, vec2(50));
    slime_ptr = new Slime(nullptr, vec2(0, 0), *player_ptr);
    ...
    GetGSComponent<GameObjectManager>()->Add(slime_ptr);
...
void Book1::Update(float dt)
{
    ...
    if (player_ptr->GetGOComponent<Life>()->Dead() == true) {
        if (Sound_One_Time == false) {
            SoundEffect::Stop_Loop_SoundEffect(SFX::Book1BGM);
...
void Book1::check_clear(float dt)
{
    if (GetGSComponent<GameObjectManager>()->GetObject<Slime>() == nullptr) {
        if (Sound_One_Time == false) {
            SoundEffect::Stop_Loop_SoundEffect(SFX::Book1BGM);
            SoundEffect::EraseSoundEffect();
